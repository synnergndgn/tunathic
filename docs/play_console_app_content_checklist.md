...
| Privacy policy | Source text is `docs/PRIVACY_POLICY.md`, effective **August 25, 2026**, contact `aligundogan2@gmail.com`; published at https://gundev.dev/gizlilik/tunathic; in-app privacy is available from Settings and About. The repo text, the published page and the in-app screen all now name the same three categories of on-device data: preferences, Repertoire songs, and Music Theory progress. | Deploy the pending `app/gizlilik/policies.ts` change in the GUNDEV site repo, confirm the live page shows the new effective date, then enter the URL. |
